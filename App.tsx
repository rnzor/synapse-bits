...
import { inferTopicSlug } from './src/utils/inferTopicSlug';
...
const INITIAL_BITS: Bit[] = INITIAL_BITS_DATA.map(b => ({
    ...b,
...
const BitDetailRoute = ({ bits, onVote, showToast, onAddXp, onQuizWin, stats, onBookmark, user, onBitComplete, onMarkCompleted, onShare }: any) => {
...
const App: React.FC = () => {
    ...
    const TracksPageWrapper = ({ bits }: any) => <TracksPage bits={bits} />;
    const TrackPageWrapper = ({ bits, stats, user }: any) => <TrackPage bits={bits} stats={stats} user={user} />;
    ...
    const [bits] = useState<Bit[]>(() => {
        return INITIAL_BITS.map(b => ({
            ...b,
            access: b.access ?? 'free',
            topicSlug: b.topicSlug ?? inferTopicSlug(b)
        }));
    });
                ...
                <div className="flex flex-col lg:flex-row gap-10">
                    ...
                    <main className="flex-1 min-w-0 pb-20">
                        <Routes>
                            ...
                            <Route path="/topic/:slug" element={<TopicPageWrapper bits={bits} stats={stats} user={user} onUpgradeModalOpen={() => setIsUpgradeModalOpen(true)} />} />
...
const ExploreView = ({
    ...
    currentPage, setCurrentPage, handleBookmark, handleVote, setSharingBit, navigate
}: any) => {
                            ...
                            {paginatedBits.map((bit: Bit) => (
                                <div key={bit.id} className="h-full">
                                    <BitCard
                                        ...
                                        onBookmark={() => handleBookmark(bit.id)}
                                    />
                                </div>
                            ))}
                        </div>
...
const TutorialsView = ({ tutorials, user }: any) => {
    return (
...
const TopicPageWrapper = ({ bits, stats, user, onUpgradeModalOpen }: any) => {
